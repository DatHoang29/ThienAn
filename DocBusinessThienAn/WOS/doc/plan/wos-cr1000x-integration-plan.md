...
| API cần dùng | `new ModbusTcpClient().Connect(ip, port, ModbusEndianness.BigEndian)` rồi `client.ReadHoldingRegistersAsync<float>(unitId, startAddress, count, cancellationToken)` — đúng như thiết kế register-map JSON ở dưới, xác nhận qua [API reference chính thức](https://apollo3zehn.github.io/FluentModbus/api/FluentModbus.ModbusClient.html) (lớp `ModbusClient`, cha của `ModbusTcpClient`). |
| ⚠️ Lưu ý khi implement | `ModbusClient` có **cả 2 bản**: `ReadHoldingRegisters<T>(...)` đồng bộ và `ReadHoldingRegistersAsync<T>(..., CancellationToken)` bất đồng bộ (trả `Task<Memory<T>>`) — dùng bản **Async** cho `WosModbusClient.ReadChannelsAsync(...)` (Phase 2), không cần tự bọc `Task.Run(...)` như đã ghi nhầm ở bản trước của tài liệu này. |
