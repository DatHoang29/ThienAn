...
tier: A
read: full
source: ../../../Plan/_source/audio/16-09-2026-sua-ui-sharedata.m4a
date: 16-09-2026
...
participants:
  - Kiên (Dev Frontend / Trình bày & Thống nhất checklist)
...
|---|---|---|---|---|
| 1 | Thêm Mã đối tác | Cấu hình Đối tác | **Kiên (FE)** | Hiển thị cột Mã đối tác bên cạnh Tên đối tác trên bảng danh sách. |
| 2 | Rào port & Bảo vệ thông tin | Cấu hình Đối tác | **Kiên (FE) / Đạt (BE)** | Khóa/Rào lại trường Port khi chỉnh sửa tên đối tác; chuẩn bị sẵn cấu trúc cho SMTP, SSE mở rộng sau. |
| 3 | Tối ưu ô gửi gần nhất & Ẩn cảnh báo | Cấu hình Đăng ký chia sẻ | **Kiên (FE)** | Lược bỏ các trường rườm rà ở ô liên kết/gửi gần nhất; ẩn box cảnh báo. |
| 4 | Chuyển đổi "Gửi khi có data mới" | Cấu hình Đăng ký chia sẻ | **Kiên (FE)** | Bỏ nút radio "Theo sự kiện", thay bằng checkbox "Gửi ngay khi có dữ liệu mới" (dựa trên `UpdateTime`/`CreateTime`). |
| 5 | Tinh giản chiều gửi & nhận | Cấu hình Đăng ký chia sẻ | **Kiên (FE)** | Chiều gửi: Bỏ định dạng, ẩn độ ưu tiên. Chiều nhận: Bỏ hẳn cấu hình kiểu lịch. |
| 6 | Thêm/Sửa/Xóa Gói tin & Trường | Cấu hình Gói tin | **Kiên (FE) / Đạt (BE)** | Không hardcode danh sách gói tin và cột nữa; cho phép CRUD động từ DB. |
| 7 | Căn chỉnh độ rộng cột & Sửa scroll | Cấu hình Gói tin | **Kiên (FE)** | Chỉnh width hợp lý cho cột Mã trường, sửa lỗi phân trang ở chân bảng; freeze cột STT hoặc Action. |
| 8 | Hiển thị theo Tệp dữ liệu | Cấu hình Gói tin | **Kiên (FE) / Đạt (BE)** | Bỏ hiển thị theo Tên bảng CSDL vật lý và bỏ cột Bí danh; gom thành Tệp dữ liệu nghiệp vụ (Data File). |
| 9 | Cải tiến UI Ánh xạ (Mapping) | Ánh xạ dữ liệu | **Kiên (FE)** | Thêm cột hiển thị option đi kèm; thay icon 3 chấm bằng icon Config; mở modal chồng khi sửa trường. |
| 10 | Lấy data mẫu thật từ DB | Ánh xạ dữ liệu | **Kiên (FE) / Đạt (BE)** | Nút gửi thử nghiệm sẽ gọi API lấy bản ghi thật trong CSDL lên để đối chiếu. |
| 11 | Bổ sung `Default Value` cho CodeSet | Ánh xạ dữ liệu | **Đạt (BE) / Kiên (FE)** | Thêm trường Giá trị mặc định (hỗ trợ phân biệt chiều gửi/nhận) khi data nguồn không khớp bảng mã. |
| 12 | Chuẩn hóa bộ chọn thời gian | Lịch sử chia sẻ | **Kiên (FE)** | Đổi sang component DateTime chuẩn; bỏ ô tìm kiếm nội dung nặng nề. |
| 13 | Đổi Sidebar thành Modal chi tiết | Lịch sử chia sẻ | **Kiên (FE)** | Khi double click dòng lịch sử, mở Modal hiển thị chi tiết thay vì Sidebar. |
| 14 | Chuẩn hóa Log truyền nhận 2 bước | Lịch sử chia sẻ & Worker | **Đạt (BE) / Kiên (FE)** | Thiết lập quan hệ cha-con: 1 phiên log chứa 2 step (Đọc DB $\to$ Gửi đi hoặc Nhận về $\to$ Lưu DB). |
| 15 | Khai báo bảng mã lỗi hệ thống | Cấu hình Hệ thống | **Đạt (BE)** | Tạo bảng quản lý danh mục Error Code chuẩn phục vụ ghi log và hiển thị trạng thái lỗi. |
...
|---|---|---|
| `00:00` | **Kiên** | Từ từ... Thứ nhất là phần cấu hình đối tác. |
| `00:03` | **Anh Sơn** | Nghe là đói bụng quá à. |
| `00:05` | **Kiên** | Thứ nhất là phần cấu hình đối tác thì ở list danh sách là sẽ thêm mã đối tác thể hiện. Thêm mã đối tác. Ở Edit modal... Edit modal tạm ổn, không cần sửa gì, đúng không? |
| `00:20` | **Anh Sơn** | Comment... Ờ, rồi. |
| `00:23` | **Kiên** | Tiếp theo... |
| `00:24` | **Anh Sơn** | Nhưng mà nhớ mở rộng nha, cái mở rộng này có nói rồi đó. |
| `00:27` | **Kiên** | Rồi, mở rộng để sau. Mở rộng để sau. Rồi tiếp theo tới phần cấu hình... Cấu hình subscription thì ở cái ô địa chỉ lần kết nối gần nhất là gửi gần nhất thì xem cấu hình lại sao cho nó hợp lý, bỏ bớt trường đi. Bỏ luôn cả box cảnh báo. |
| `00:48` | **Kiên** | Rồi còn ở trong Edit cấu hình subscription thì... Thì sao anh Sơn, quên mất tiêu rồi? |
| `00:58` | **Kiên** | À... Button gửi ngay khi có dữ liệu mới chứ không phải là theo sự kiện. Hằng ngày thì thêm một cái chu kỳ... |
| `01:13` | **Anh Sơn** | Cái gì cái gì? Chu kỳ gì? |
| `01:16` | **Kiên** | Ý là... |
| `01:17` | **Anh Sơn** | Nói hồi không biết đang nói cái gì luôn! |
| `01:19` | **Kiên** | Đi vòng vòng một hồi em quay lại cái tự nhiên em bị mất trí nhớ tạm thời! |
| `01:23` | **Kiên** | Rồi, liên tục là chuẩn rồi nè đúng không? Ở kiểu lịch á thì liên tục là chuẩn rồi. |
| `01:30` | **Anh Sơn** | Thì hiện tại là chuẩn rồi, nhưng mà mốt thêm cái thời gian này kia... |
| `01:34` | **Kiên** | Thôi giờ nói hiện tại thôi, để mốt... |
| `01:37` | **Anh Sơn** | Nói chung là vậy, nhưng mà chỉnh lại cái giao diện cho nó phù hợp. |
| `01:42` | **Kiên** | Rồi, hằng ngày thì... |
| `01:45` | **Anh Sơn** | Trời ơi, nữa! |
| `01:47` | **Kiên** | Bỏ đi định dạng, bỏ ưu tiên... bỏ định dạng, bỏ ưu tiên đi. Còn chiều nhận về thì sẽ bỏ đi kiểu lịch, hiện tại là cứ để nhận về thôi. |
...
| `02:09` | **Kiên** | Không, em thấy có chút xíu à. |
| `02:11` | **Anh Sơn** | Nhiều quá! |
| `02:12` | **Kiên** | Ở phần cấu hình gói tin thì cho thêm sửa xóa... Thêm sửa xóa gói tin, thêm sửa xóa các cột dữ liệu. |
| `02:22` | **Anh Sơn** | Rồi chỉnh lại cái... cái dòng đầu nữa kìa. Chỉnh lại cái giao diện kìa. |
| `02:27` | **Kiên** | Chỉnh lại cái giao diện của cấu hình gói tin. Chỉnh lại cái phần phân trang ở dưới bị lỗi. |
...
| `02:49` | **Kiên** | Cột đầu nó đang bị rác, bị gác cái gì đó... |
...
| `03:13` | **Kiên** | Không, fix... fix fix sườn... fix sườn... |
...
| `04:18` | **Kiên** | Cấu hình gói tin... Cấu hình gói tin hiển thị theo từng tệp data, không hiển thị theo tên bảng. Mới hiểu luôn á, mới biết luôn á! |
| `04:26` | **Kiên** | Rồi, Mapping nè. Mapping ở phần cấu hình ánh xạ thì thêm một cột thể hiện những cái option đi kèm. Thiết kế lại UI/UX của phần ánh xạ gói tin. |
| `04:40` | **Anh Sơn** | Ừ. |
| `04:41` | **Kiên** | Dữ liệu gửi thử thì lấy data từ dưới DB lên. |
| `04:46` | **Anh Sơn** | Ừ, bấm nút gửi hay là bấm nút lấy gì đó... lấy dữ liệu mẫu gì đó. |
| `04:52` | **Kiên** | Thông tin chung ổn, bỏ đi định dạng, bỏ bỏ phiên bản. Ở phần bộ mã chuẩn hóa thì thêm một cái... thì thêm một một một ô Default là... |
| `05:12` | **Anh Sơn** | Là... là... là giá trị... |
| `05:15` | **Kiên** | Là giá trị mặc định. Nếu mà dữ liệu không có... |
| `05:19` | **Kiên** | Nếu dữ liệu không có thì thêm một... thì luôn luôn có một ô Default là dữ liệu... là dữ liệu mặc định. |
| `05:25` | **Anh Sơn** | Nhưng mà dữ liệu mặc định thì xem coi là dữ liệu chiều nào. |
| `05:32` | **Kiên** | Default Value sẽ thêm chiều của CodeSet, cấu hình gói tin CodeSet sẽ thêm chiều cho Default Value. Ở phần lịch sử thì sẽ chỉnh sửa lại khung thời gian, chỉnh sửa lại tên tên của các ô lựa chọn tìm kiếm: đối tác, đối tượng này. Ở tab truyền nhận thì... ở phần chi tiết cân nhắc thể hiện panel. Thêm các step cho từng bước nhận đi, gửi về. |
| `06:01` | **Anh Sơn** | Step đó là cái gì? |
| `06:03` | **Kiên** | Ví dụ step gửi đi thì sẽ có là lấy data từ dưới DB lên xử lý rồi gửi đi. Step nhận về là tiếp nhận gói tin, xử lý rồi lưu DB. Khai báo cấu hình... cấu hình mã lỗi ở... khai báo mã lỗi, khai báo bảng mã lỗi ở trong bảng cấu hình hệ thống. |
