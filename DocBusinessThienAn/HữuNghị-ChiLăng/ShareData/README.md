...
| A | `Prompt/sharedata-ghi-tep-khong-chan-gui-api-prompt.md` | full | 8 KB | **Prompt dùng-1-lần đang mở** — Lỗi ghi tệp đang `return` sớm nên **API không bao giờ được gọi**; đĩa đầy là đối tác không nhận được gì. Bỏ `return`, để cả hai kênh cùng chạy. Kèm bảng quyết trạng thái 6 ô: `Failed` **chỉ khi không kênh nào** thành công | Biên soạn nội bộ |
