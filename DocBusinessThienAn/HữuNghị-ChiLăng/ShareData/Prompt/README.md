...
| Prompt | Kết quả |
|---|---|
| `sharedata-fix-releaselock-finally-va-log-getcurrentdbversion-prompt.md` | ✅ **26/09/2026** · Đưa toàn bộ try/catch và log của `ReleaseLock` vào bên trong chính hàm `ReleaseLock` (khối `finally` của `ExportSubscription` tinh gọn còn 1 dòng); ghi log debug khi `GetCurrentDbVersion` không đọc được version; rút gọn `CheckTrackingDb`; chuyển `CHANGE_RETENTION = 1 DAYS`; rút gọn tên hàm `IsChangeTrackingVersionInvalid` thành `IsCtVersionInvalid` (giữ alias `IsVersionInvalid` và `IsChangeTrackingVersionInvalid`); toàn bộ test ShareData PASS 100% |
