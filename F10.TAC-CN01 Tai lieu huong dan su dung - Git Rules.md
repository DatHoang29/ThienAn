> 📌 **Vị trí tài liệu chính thức (SSOT):** Tài liệu này đã được chuẩn hóa và đưa vào hệ thống tài liệu chung tại:  
> 👉 [`DocBusinessThienAn/QuyDinhChung/doc/F10.TAC-CN01-git-rules.md`](DocBusinessThienAn/QuyDinhChung/doc/F10.TAC-CN01-git-rules.md)

# CÔNG TY CỔ PHẦN ĐẦU TƯ CÔNG NGHỆ THIÊN ÂN

![Logo Thiên Ân Corp](docs/images/git-rules/image1.png)
...
- **Email:** Sử dụng địa chỉ email công ty.

![Thông tin tài khoản Git](docs/images/git-rules/image2.png)
...
> **Lưu ý:**
> - Nên thêm tên nhân sự vào để dễ dàng quản lý.
> - Thời gian rule: `yyyyMMdd` hoặc `yyyyMM`.
> - Tên branch viết tiếng Việt không dấu / tiếng Anh.

#### Thứ tự thực hiện:

![Mô hình Git Flow](docs/images/git-rules/image3.png)
...
![Commit Message Types](docs/images/git-rules/image4.png)
...
> **Lưu ý:** Thêm dấu **`!`** để nhấn mạnh thay đổi lớn có thể ảnh hưởng hệ thống / luồng xử lý cũ (Breaking Change).
...
- **Tham khảo Pull Request & Xử lý xung đột:**

![Pull Request Conflict](docs/images/git-rules/image5.png)

Trường hợp pull request bị báo lỗi xung đột, tiến hành các bước sau:
1. Nhánh `staging` pull request lên `release` bị xung đột.
2. Tiến hành lấy nhánh `staging` về. Thực hiện merge `release` vào `staging`.
3. Giải quyết xung đột ở nhánh `staging` và commit lên.
4. Refresh lại trang pull request và chấp thuận pull request.
