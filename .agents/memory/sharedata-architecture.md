...
- `DataNatsConsumerWorker`: Đăng ký NATS subject `ta.its.event.sharedata.newdata`, parse PacketCode/Type và gọi `DataOutboundService.ProcessPacketTrigger` để kích hoạt xuất bản các subscription có `SendOnNewData = true`.
