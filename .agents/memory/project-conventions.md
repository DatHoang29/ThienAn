...
- **Command & Validator Integration Testing (No Separate Validator Test Files)**: Khi viết kiểm thử tích hợp cho TẤT CẢ các Command (Add, Update, Delete, BatchDelete, Ping, Probe, Sync, Setup, Reset, Workflow commands...) có Validator (FluentValidation), BẮT BUỘC phải kiểm thử validator trực tiếp trong luồng test của class test Controller/Command tương ứng (khởi tạo `var validator = new XxxValidator();`). TUYỆT ĐỐI KHÔNG TÁCH CLASS/FILE TEST VALIDATOR RIÊNG BIỆT. Luồng hợp lệ: gọi `validator.ValidateAsync(input)` và assert `Assert.True(valResult.IsValid)` trước khi gửi qua MessageBus. Luồng lỗi: kiểm tra Negative Rule (`Assert.False(invalidResult.IsValid)`) trực tiếp trong bài test tương ứng.
