...
- **XML COMMENT CONVENTION FOR HANDLERS/DTO/VALIDATORS/TESTS**: Tất cả các lớp và phương thức trong `Commands/`, `Queries/`, `Dto/`, `Validators/` và toàn bộ phương thức kiểm thử Test (`[Fact]` / `[Theory]` trong `tests/`) BẮT BUỘC phải có comment XML `/// <summary>` theo thứ tự chuẩn: `Author: Đạt` ở dòng đầu, `Description: <Mô tả chức năng/nghiệp vụ>` ở giữa, và `Created date: DD/MM/YYYY` ở dòng cuối. BỎ HẲN và KHÔNG DÙNG field `Updated date:`.
