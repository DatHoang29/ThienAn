...
- **DO NOT KILL GENERAL `dotnet` PROCESSES / MOCKSERVER / WEBAPI (WPF Process Termination Rule)**: Mỗi lần rebuild, re-run hoặc chạy test liên quan tới module WPF, nếu cần giải phóng file DLL/EXE bị lock, **CHỈ ĐƯỢC PHÉP tắt riêng đúng tiến trình WPF** (`Get-Process -Name "Module.VideoWall.WPF" -ErrorAction SilentlyContinue | Stop-Process -Force`). TUYỆT ĐỐI KHÔNG kill diện rộng như `Stop-Process -Name "dotnet"`, `Get-Process testhost*,dotnet* | Stop-Process`, `taskkill /f /im dotnet.exe` (để tránh làm tắt nhầm `dotnet MockServer`, `TAC_WebAPI`, hoặc background worker đang chạy của người dùng).
