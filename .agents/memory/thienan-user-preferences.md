...
- **SINGLE-STATEMENT IF FORMATTING (Không tự ý thêm `{}`)**: Đối với câu lệnh `if` chỉ chứa 1 dòng thực thi (VD: `if (await _vwWindowSceneRep.IsAnyAsync(u => u.IsDelete == null && u.Code == command.Code && u.ID != command.ID))\n    throw Oops.Oh(BaseLocaleManager.BaseException.Exist, BaseLocaleManager.BaseEntity.Code);` hoặc `if (targetSubs.Count == 0)\n    return;`), BẮT BUỘC ngắt dòng và thụt lề cho câu lệnh thực thi. TUYỆT ĐỐI KHÔNG VIẾT NGANG TRÊN CÙNG 1 DÒNG (`if (...) return;`) và TUYỆT ĐỐI KHÔNG TỰ Ý THÊM cặp dấu ngoặc nhọn `{}` khi code hiện hữu đang viết theo chuẩn single-statement không có `{}`.
