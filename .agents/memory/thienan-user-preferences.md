...
- **DO NOT KILL GENERAL `dotnet` PROCESSES / MOCKSERVER**: Khi cần tắt tiến trình bị lock file DLL hoặc tắt app để build/test, TUYỆT ĐỐI KHÔNG dùng lệnh `Stop-Process -Name "dotnet"` (vì sẽ tắt nhầm `dotnet MockServer` đang chạy ngầm của người dùng). CHỈ ĐƯỢC PHÉP tắt riêng tiến trình app WPF (`Module.VideoWall.WPF*`, `testhost*`).
