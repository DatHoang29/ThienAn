...
metadata: 
  ...
  created: 2026-07-21
  updated: 2026-07-29
  originSessionId: 1826fe3f-b3f0-44bd-9099-8e2c8c564b85
  modified: 2026-09-05T04:09:39.760Z
---
...
- **MANDATORY TEST VERIFICATION & NEW TEST COVERAGE ON CODE/UI/LOGIC CHANGES**: Bất cứ khi nào tạo mới hoặc sửa code (C#, XAML, ViewModel, Service, Handler, Controller, API...), thêm mới UI (component, view, layout, control, button, validator...), hoặc sửa đổi logic nghiệp vụ hay logic giao diện: AI **BẮT BUỘC** phải:
  1. **Kiểm tra chạy lại toàn bộ bài test liên quan (`dotnet test ...`)** để đảm bảo 100% test cases pass và không bị lỗi biên dịch, gãy giao diện hoặc hồi quy (regression).
  2. **Viết bổ sung test case mới** nếu tính năng/luồng logic/UI logic mới chưa có bài test bao phủ (tuân thủ nghiêm ngặt chuẩn AAA, kiểm thử luồng thực tế và mock I/O HTTP/thiết bị, đặt tên file và thư mục mirror 1-1).
  *Nhiệm vụ chỉ được coi là hoàn tất khi đã có đầy đủ test case bao phủ và chạy test pass 100%.*
  - **DỒN TEST VỀ CUỐI KHI ĐANG TRAO ĐỔI DỒN DẬP (xác nhận 05/09/2026)**: Nếu đang trong 1 chuỗi hỏi-đáp/sửa nhanh liên tiếp (nhiều quyết định nhỏ dồn dập, rõ ràng còn quyết định khác sắp tới) và bài test liên quan chạy chậm (test suite lớn, ví dụ ~60s+), KHÔNG chạy lại `dotnet test` sau MỖI lần sửa nhỏ — dồn các thay đổi liên quan lại và chỉ chạy test 1 lần ở cuối, ngay trước khi báo cáo hoàn tất/dừng lại. Vẫn chạy test ngay lập tức nếu: (a) người dùng hỏi trực tiếp "test chưa/pass chưa", (b) thay đổi đủ rủi ro cần xác nhận trước khi tiếp tục dựa vào kết quả đó, hoặc (c) không còn gì khác đang chờ quyết định (rõ ràng đây là bước cuối). Lý do: tránh lãng phí nhiều phút chờ test lặp lại khi người dùng đang hỏi dồn nhiều câu liên tiếp.
- **DO NOT KILL GENERAL `dotnet` PROCESSES / MOCKSERVER / WEBAPI (WPF Process Termination Rule)**: Mỗi lần rebuild, re-run hoặc chạy test liên quan tới module WPF, nếu cần giải phóng file DLL/EXE bị lock, **CHỈ ĐƯỢC PHÉP tắt riêng đúng tiến trình WPF** (`Get-Process -Name "Module.VideoWall.WPF" -ErrorAction SilentlyContinue | Stop-Process -Force` hoặc PID lock DLL). TUYỆT ĐỐI KHÔNG kill diện rộng như `Stop-Process -Name "dotnet"`, `Get-Process testhost*,dotnet* | Stop-Process`, `taskkill /f /im dotnet.exe`. Người dùng sẽ tự chủ động khởi chạy ứng dụng WPF khi cần, AI không tự ý chạy `dotnet run`.
...
- **XML COMMENT CONVENTION FOR HANDLERS/DTO/VALIDATORS/TESTS**: Tất cả các lớp và phương thức trong `Commands/`, `Queries/`, `Dto/`, `Validators/` và toàn bộ phương thức kiểm thử Test (`[Fact]` / `[Theory]` trong `tests/`) BẮT BUỘC phải có comment XML `/// <summary>` theo thứ tự chuẩn: `Description: <Mô tả chức năng/nghiệp vụ>` ở đầu, và `Created date: DD/MM/YYYY` ở cuối. KHÔNG DÙNG field `Author:` (quyết định 05/09/2026 — bỏ Author, giữ Created date) và KHÔNG DÙNG field `Updated date:`.
...
- **AGENT/SKILL ANNOUNCEMENT PROTOCOL — PHẢI CLASSIFY TRƯỚC KHI TRẢ LỜI (xác nhận 05/09/2026)**: Trước khi trả lời, AI PHẢI tự phân loại yêu cầu theo bảng classifier trong `.agents/rules/request-routing.md`. **QUESTION/SURVEY thuần** ("là gì", "khác nhau sao", giải thích, liệt kê/phân tích không sửa code) → trả lời thẳng bằng text, KHÔNG cần banner. **SIMPLE CODE / COMPLEX CODE / DESIGN** (sửa code, viết prompt fix, refactor, review đúng/sai 1 thay đổi code...) → BẮT BUỘC mở đúng file `.agents/agent/{agent}.md` liên quan trước, announce `🤖 Applying knowledge of @[agent-name]...` (và `📚 Using skill: @[skill-name]...` nếu load skill từ frontmatter agent đó) rồi mới trả lời. Lý do: đây là rule P0 trong `.agents/rules/core-protocol.md` + `request-routing.md` nhưng từng bị bỏ qua nhiều lượt (VD: "kiểm tra test trùng lặp... cho prompt fix" và "kiểm tra code change có đúng mô tả chưa" đều là COMPLEX CODE/verification lẽ ra phải announce `@test-engineer` nhưng đã bị xử lý như 1 phiên Claude Code thường, không route qua persona).
...
- **Class Header XML Comments Required**: Every new or updated class MUST include an XML summary header block formatted as follows (ONLY `Created date:` — NO `Author:` since 05/09/2026, NO `Updated date:`):
```csharp
/// <summary>
